...
void test_entry(void) {
	...
	bmfs_assert(sizeof(entry) == BMFS_ENTRY_SIZE);
	...
	/* Name */
	bmfs_assert(memcmp(&buf[0], "filename.txt", sizeof("filename.txt") - 1) == 0);
	/* Size */
	bmfs_assert(memcmp(&buf[192], "\x01\x00\x00\x00\x00\x00\x00\x00", 8) == 0);
	/* Offset */
	bmfs_assert(memcmp(&buf[192 + 8], "\x02\x00\x00\x00\x00\x00\x00\x00", 8) == 0);
	/* Flags */
	bmfs_assert(memcmp(&buf[192 + 16], "\x03\x00\x00\x00\x00\x00\x00\x00", 8) == 0);
	/* Type */
