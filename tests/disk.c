...
int main(void)
{
	...
	err = bmfs_disk_format(disk, buf_size);
	assert(err == 0);

	/* Test the creation of directories */

	err = bmfs_disk_create_dir(disk, "/tmp");
	assert(err == 0);

	/* Test the creation of files */

	err = bmfs_disk_create_file(disk, "/tmp/a.txt", 1);
	printf("err: %s\n", strerror(-err));
	assert(err == 0);
	...
	err = bmfs_disk_create_file(disk, "/tmp2/b.txt", 1);
	assert(err != 0);
